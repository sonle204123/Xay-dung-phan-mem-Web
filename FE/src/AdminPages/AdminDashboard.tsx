...
// IMPORT TẤT CẢ CÁC COMPONENT ĐÃ TẠO
...
const AdminDashboard: React.FC = () => {
  ...
  const userRole = user?.role_id || 0; // 1: Admin, 2: Bác sĩ, 3: Lễ tân

  // 2. CẤU HÌNH MENU PHÂN QUYỀN (Thêm Patient và DoctorExam vào đây)
  const MENU_ITEMS = [
    ...
    { id: 'exam', label: 'Khám bệnh & Điều trị', icon: '🩺', allowedRoles: [1, 2] }, // Bác sĩ và Admin
  ...
  // Bảo mật: Nếu không có quyền nào (không phải nhân viên), đá văng ra ngoài trang chủ
  useEffect(() => {
    ...
    <div className="flex h-screen bg-slate-50 font-sans">
      ...
      <div className="w-64 bg-slate-900 text-white flex flex-col">
        ...
        <ul className="flex-1 px-4 py-6 space-y-2 overflow-y-auto">
          {/* TỰ ĐỘNG VẼ MENU DỰA TRÊN QUYỀN */}
        ...
        <div className="bg-white p-4 border-b border-slate-200 flex justify-between items-center shadow-sm z-10">
          ...
          <div className="flex items-center gap-4">
            <div className="text-right">
              ...
              <div className="text-xs font-bold text-blue-600 bg-blue-50 inline-block px-2 py-0.5 rounded-full mt-1">
                {userRole === 1 ? 'QUẢN TRỊ VIÊN' : userRole === 2 ? 'BÁC SĨ' : 'LỄ TÂN'}
              </div>
            </div>
            <div className="w-10 h-10 bg-blue-600 text-white rounded-full flex items-center justify-center font-bold shadow-md border-2 border-blue-200">
